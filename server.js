...
// Folder gudang uploads lokal di Railway
...
// Simpan Gambar ke Gudang Railway untuk Nano Banana (Harus URL)
...
function hapusFileGudang(urlLengkap) {
    try {
        ...
        if (fs.existsSync(jalurFisik)) fs.unlinkSync(jalurFisik); 
    } catch (e) { console.error("Gagal hapus file:", e); }
}

// ==========================================
// PINTU UTAMA: GENERATE MAGNIFIC
// ==========================================
app.post('/generate', upload.fields([{ name: 'foto1' }, { name: 'foto2' }, { name: 'foto3' }]), async (req, res) => {
    ...
    try {
        ...
        if (!req.files || !req.files['foto1']) return res.status(400).json({ status: "Error", pesan: "Gambar utama wajib ada." });
        ...
        if (fitur === 'upscale') {
            ...
            TARGET_URL = fitur === 'ugc-flash' ? 'https://api.magnific.com/v1/ai/text-to-image/nano-banana-pro-flash' : 'https://api.magnific.com/v1/ai/text-to-image/nano-banana-pro';
            ...
            // Fix Rasio Anti-Miring
            let bananaRatio = ratio === "9:16" ? "9:16" : "1:1";
            ...
            if (req.files['foto2']) arrayGambarGemini.push(await konversiKeBase64Steril(req.files['foto2'][0].buffer));
            if (req.files['foto3']) arrayGambarGemini.push(await konversiKeBase64Steril(req.files['foto3'][0].buffer));
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Server HERRA MAGNIFIC aktif di port ${PORT}`));
